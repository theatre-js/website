...
You can create a project with the [getProject](/docs/0.5/api/core#getproject) function in `@theatre/core`.
...
To export the project, click on the name of the project in the [Outline Panel](/docs/0.5/manual/Studio#outline-panel) of the studio and then click on the "Export [Project's name]" button in the top right corner of the screen as seen below.

<Video src="/images/docs/0.5/manual/projects/01-save-a-project.mp4">Saving a project's state</Video>

You can load a project's state from a JSON file by passing the state down as an object in the configuration of [getProject](/docs/0.5/api/core#getproject):
...
Currently, the configuration object of [getProject](/docs/0.5/api/core#getproject) consists only of the `state` property.

When using Studio, loading a project happens asynchronously. You can wait for the project to load through [Project.ready](/docs/0.5/api/core#project.ready) and [Project.isReady](/docs/0.5/api/core#project.isready)
...
Learn more about related API at [Project API docs](/docs/0.5/api/core#project).
