...
Sheets contain one or more [Objects](/docs/0.5/manual/objects), that can be animated together.

## Creating Sheets

You can create a Sheet with the [Project.sheet](/docs/0.5/api/core#project.sheet) function in `@theatre/core`.
...
Each Sheet has a [Sequence](/docs/0.5/manual/sequences) attached to it.
You can access a Sheet's [Sequence](/docs/0.5/manual/sequences) through [Sheet.sequence](/docs/0.5/api/core#sheet.sequence).
You can then use the [playback controls](/docs/0.5/api/core#sequence) on the Sequence to play back the animation.
...
Theatre.js supports this use case, through Sheet Instances. You can create an instance of a sheet by passing an optional instance id as the second argument to [Project.sheet](/docs/0.5/api/core#project.sheet).
...
Learn more about related API at [Sheet API docs](/docs/0.5/api/core#sheet).
